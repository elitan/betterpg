...
bpg snapshot create $TESTBRANCH --label before-test-1
...
sleep 2
bpg snapshot create $TESTBRANCH --label before-test-2
...
bpg branch create $TESTDB/pitr-test --pitr "$TARGET_TIME"
echo ""
...
PITR_PORT=$(jq -r '.databases[] | select(.name=="'$TESTDB'") | .branches[] | select(.name=="'$TESTDB'/pitr-test") | .port' ~/.local/share/betterpg/state.json)
...
bpg branch create $TESTDB/pitr-snapshot1 --pitr "$SNAPSHOT1_TIME"
echo ""
...
PITR_PORT2=$(jq -r '.databases[] | select(.name=="'$TESTDB'") | .branches[] | select(.name=="'$TESTDB'/pitr-snapshot1") | .port' ~/.local/share/betterpg/state.json)
...
if bpg branch create $TESTDB/pitr-error --pitr "$BEFORE_TIME" 2>&1 | grep -q "No snapshots found"; then
