...
DEV_PORT=$(cat $STATE_FILE | jq -r '.projects[] | .branches[] | select(.name == "test-db/dev") | .port')
...
CONN_STRING=$(cat $STATE_FILE | jq -r '.projects[0].credentials | "postgresql://\(.username):\(.password)@localhost:\(.port // 5432)/\(.database)"')
MAIN_PORT=$(cat $STATE_FILE | jq -r '.projects[0].branches[0].port')
CONN_STRING_WITH_PORT=$(cat $STATE_FILE | jq -r '.projects[0].credentials | "postgresql://\(.username):\(.password)@localhost:'$MAIN_PORT'/\(.database)"')
...
BRANCH_COUNT=$(cat $STATE_FILE | jq '.projects[0].branches | length')
...
DUPLICATE_COUNT=$(cat $STATE_FILE | jq '[.projects[0].branches[].name] | group_by(.) | map(length) | max')
...
STATE_DATASETS=$(cat $STATE_FILE | jq -r '.projects[0].branches[].zfsDatasetName' | sort)
...
SNAPSHOT_COUNT=$(cat $STATE_FILE | jq '[.snapshots[] | select(.projectName == "test-db")] | length')
...
RUNNING_BRANCHES=$(cat $STATE_FILE | jq -r '.projects[0].branches[] | select(.status == "running") | .containerName')
...
CONTAINER_NAME=$(cat $STATE_FILE | jq -r '.projects[0].branches[0].containerName')
...
if ! cat $STATE_FILE | jq -e '.projects[0].branches[] | select(.name == "test-db/branch-1")' >/dev/null 2>&1; then
    STATE_GONE=true
else
    STATE_GONE=false
fi
