...
DEV_PORT=$(cat ~/.local/share/betterpg/state.json | jq -r '.databases[] | .branches[] | select(.name == "test-db/dev") | .port')
...
CONN_STRING=$(cat ~/.local/share/betterpg/state.json | jq -r '.databases[0].credentials | "postgresql://\(.username):\(.password)@localhost:\(.port // 5432)/\(.database)"')
MAIN_PORT=$(cat ~/.local/share/betterpg/state.json | jq -r '.databases[0].branches[0].port')
CONN_STRING_WITH_PORT=$(cat ~/.local/share/betterpg/state.json | jq -r '.databases[0].credentials | "postgresql://\(.username):\(.password)@localhost:'$MAIN_PORT'/\(.database)"')
...
DUPLICATE_COUNT=$(cat ~/.local/share/betterpg/state.json | jq '[.databases[0].branches[].name] | group_by(.) | map(length) | max')
...
STATE_DATASETS=$(cat ~/.local/share/betterpg/state.json | jq -r '.databases[0].branches[].zfsDatasetName' | sort)
...
SNAPSHOT_COUNT=$(cat ~/.local/share/betterpg/state.json | jq '[.snapshots[] | select(.databaseName == "test-db")] | length')
...
RUNNING_BRANCHES=$(cat ~/.local/share/betterpg/state.json | jq -r '.databases[0].branches[] | select(.status == "running") | .containerName')
...
CONTAINER_NAME=$(cat ~/.local/share/betterpg/state.json | jq -r '.databases[0].branches[0].containerName')
if echo "$CONTAINER_NAME" | grep -q "^bpg-test-db-main$"; then
    check_test "Container naming convention (bpg-<db>-<branch>)"
else
...
if ! docker ps -a | grep -q "bpg-test-db-branch-1"; then
    CONTAINER_GONE=true
else
...
if ! cat ~/.local/share/betterpg/state.json | jq -e '.databases[0].branches[] | select(.name == "test-db/branch-1")' >/dev/null 2>&1; then
    STATE_GONE=true
else
    STATE_GONE=false
fi
