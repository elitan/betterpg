...
export async function branchRenameCommand(oldName: string, newName: string) {
  console.log();
  console.log(`Renaming branch ${chalk.bold(oldName)} to ${chalk.bold(newName)}...`);
  console.log();
  ...
  console.log('Warning: Renaming requires stopping the branch, updating the container, and restarting.');
